...
def SplitModeClips(tf,ma,re=True):
    ...
    This is then sorted by bout, then by start node. So you'll have the first bouts first, 
    then inside those, the clips will be sorted by time. So basically, the clips will be sorted
    by time.
        ...
        if len(b)>1: # ignore bouts with just the exit state
            ...
            if k>=0:
                while edb[k]>edb[k+1]: # as long as distance increases
                    ...
                    if k==-1: # stop at the start of the bout
                        break
            k+=1 # this is the first state in this monotonic path toward the exit  
