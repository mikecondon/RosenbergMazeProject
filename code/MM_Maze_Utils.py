...
def StepType4(i,j,m):
    '''
    Make same as StepType2 but make transitions from an end node to its preceding node
    equal to 4. Entering the maze is indicated by 3 and exiting the maze is indicated
    by 5.
    in left = 0
    in right = 1
    '''
    # entering the maze (start of bout). i=128, j=0
    if i==128:
        return 4
    # leave when j=128
    if j==128:
        return None
    st4 = m.st[i, j]
    # make both 'out left' and 'out right' = 2
    '''if st4==3: 
        st4=2'''
    # if coming from an end node, action value is 4 to signify reached bottom
    if i>=63 and i<128:
        st4=5
    return st4
...
class BinaryTree:
    ...
    def move(self, pos, dir):
        ...
        if dir==2 or dir==4:
            ...
            return self.into(pos, dir)
    
    def out(self, pos):
        if pos == 0:
            # -1 denotes illegal value
            return -1
        else:
            return (pos - 1) // 2
            
    def into(self, pos, dir):
        if pos >= 2**self.depth-1:
            return -1
...
def Action2Node(actions, depth=6):
    '''
    Action to String converter
    need a binary tree object where parent gives parent node number, 
    and appropriate for left/right child
    '''
    action_list = actions.strip('3').split('3')
    bt = BinaryTree(6)
    bouts = []
    for path in action_list:
        bout = [0]
        for action in path:
            bout.append(bt.move(bout[-1], int(action)))
        bouts.append(bout)
    return bouts
